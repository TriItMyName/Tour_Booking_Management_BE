...
function initTabs() {
  tabButtons.forEach((btn, idx) => {
    btn.addEventListener('click', async (e) => {
      ...
      if (idx === 1) {
        await loadCustomerSchedule(false);
        forceTimelineSync();
        startTimelineTicker();
      }
    });
...
async function bookAndCloseTour(tourId, userId) {
  const res = await fetch(`${API_BASE_URL}/bookings/book-and-close`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ tour_id: Number(tourId), user_id: Number(userId) })
  });
...
/* Tours (OPEN) */
function initTourListControls() {
  document.getElementById('customer-tour-search-btn')?.addEventListener('click', () => {
    loadOpenTours(document.getElementById('customer-tour-search-input')?.value || '');
  });

  document.getElementById('customer-tour-search-input')?.addEventListener('keydown', (e) => {
    if (e.key === 'Enter') {
      e.preventDefault();
      loadOpenTours(document.getElementById('customer-tour-search-input')?.value || '');
    }
  ...
  document.getElementById('customer-tour-refresh-btn')?.addEventListener('click', () => {
    const q = document.getElementById('customer-tour-search-input');
    if (q) q.value = '';
    loadOpenTours();
  });
}

async function loadOpenTours(search = '') {
  try {
    setTableMessage('customer-show-tours', 7, 'Đang tải dữ liệu...');

    let tours = await fetchTours();
    tours = tours.filter((t) => normalizeStatus(t.status) === 'OPEN');
    ...
    renderOpenTours(tours);
  } catch (err) {
    console.error('Customer tours load error', err);
    setTableMessage('customer-show-tours', 7, 'Lỗi tải dữ liệu');
  }
}

function renderOpenTours(tours) {
  const tbody = document.getElementById('customer-show-tours');
  if (!tbody) return;

  if (!tours.length) {
    setTableMessage('customer-show-tours', 7, 'Không có tour đang mở');
    return;
  }
  ...
  tbody.innerHTML = tours.map((t) => {
    ...
    return `<tr>
      ...
      <td>${escapeHtml(t.region_name || mapRegion(t.region_id))}</td>
      <td>
        <button class="btn-action approve" type="button" data-book="1" data-id="${escapeHtml(tourId)}">
          Đặt tour
        </button>
      </td>
    </tr>`;
  ...
  tbody.querySelectorAll('button[data-book]')?.forEach((btn) => {
    btn.addEventListener('click', () => openBookingConfirmModal(btn.getAttribute('data-id') || ''));
  });
}

/* Modal confirm booking */
function initBookingConfirmModal() {
  const modal = document.getElementById('customer-booking-confirm-modal');
  if (!modal) return;

  document.getElementById('customer-booking-confirm-close')?.addEventListener('click', closeBookingConfirmModal);
  document.getElementById('customer-booking-confirm-no')?.addEventListener('click', closeBookingConfirmModal);

  document.getElementById('customer-booking-confirm-yes')?.addEventListener('click', async () => {
    try {
      const tourId = Number(_pendingBookTourId || 0);
      const userId = getLoggedInUserId();
      if (!tourId || !userId) return closeBookingConfirmModal();

      await bookAndCloseTour(tourId, userId);
...
function openBookingConfirmModal(tourId) {
  _pendingBookTourId = tourId;

  const modal = document.getElementById('customer-booking-confirm-modal');
  const msg = document.getElementById('customer-booking-confirm-message');
  if (!modal || !msg) return;

  msg.textContent = 'Xác nhận đặt Tour?';
...
function initScheduleControls() {
  document.getElementById('customer-schedule-refresh-btn')?.addEventListener('click', async () => {
    await loadCustomerSchedule(true);
    forceTimelineSync();
    startTimelineTicker();
  });
}

async function fetchScheduleByUser(userId) {
  const res = await fetch(`${API_BASE_URL}/tour-timeline/schedule/user/${Number(userId)}`);
  const json = await res.json().catch(() => ({}));
  if (!res.ok) throw new Error(json.error || json.message || 'Request failed');
  return Array.isArray(json) ? json : (json.data || []);
}
...
async function loadCustomerSchedule(forceReload = false) {
  ...
  try {
    ...
    const schedule = await fetchScheduleByUser(userId);
    ...
    const enriched = await Promise.all(schedule.map(async (t) => {
      ...
      return { ...t, tour_id: tourId, timeline: Array.isArray(timeline) ? timeline : [] };
    }));

    _scheduleCache = enriched;
    renderCustomerSchedule(_scheduleCache);
...
function renderCustomerSchedule(items) {
  const box = document.getElementById('customer-schedule-list');
  if (!box) return;
  ...
  box.innerHTML = items.map((t) => {
    ...
    const sub = `Từ ${formatDate(t.start_date)} đến ${formatDate(t.end_date)} • ${regionName} • Tối đa: ${t.max_participants ?? 'N/A'}`;
